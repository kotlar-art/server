...
public class Tests implements Runnable {
    ...
    public String testCourseStat(){
        ...
        if (!ack.equals("ACK 7")){
            System.out.print("your ack is not alright!! you had: ");
            System.out.println(ack);
            System.out.println("it;s length is:" + ack.length());
            System.out.println("and you should have had:" + "ACK 7 with length 5");
            return "ack is not alright - FAILED coursestat TEST";
        }
        if (!courseName.equals("Course: ("+p.getCourseNum()+") "+p.getCourseName())){
            System.out.println(courseName);
            return "courselist is not alright - FAILED coursestat TEST";
        }
        if (!seats.equals("Seats Available: "+"0/"+numOfThreads)){
            System.out.println(seats);
            return "no. of seats is not alright - FAILED coursestat TEST";
        }
        if (!studentsL.equals(students)){
            System.out.println(studentsL);
            return "student list is not alright - FAILED coursestat TEST";
        }
        CommandProcessor commandProcessorOfStudent=new CommandProcessor();
        commandProcessorOfStudent.initialize();
        commandProcessorOfStudent.sendCommand("LOGIN RON0 RON0");
        ...
        if (!ack1.equals("ACK 7")||!courseName1.equals("Course: ("+p.getCourseNum()+") "+p.getCourseName())
            ...
            return "secomddddd CourseStat Test - Failed";
        return "thirddddddd CourseStat Test - Passed";
    ...
    public void generateCourses(){
        try {
            ...
            for (int i = 0; i < numCoursesToGenerate; i++) {
                int generatedCourseNum = ((int) (Math.random() * 100));
    ...
    public void run() {
        try {
            ...
            new Thread(() -> {
                ReactorMain.main(new String[]{"7777", "4"}); //You can change it to reactor also
            }).start();
